...
The goal of macrodata is to provide access to international economic and social panel data from major organizations and academic researchers. It includes functions for loading, processing, and harmonizing data across different sources and frequencies.
...
```{r processing-example, eval = FALSE}
...
```{r aggregation-example, eval = FALSE}
...
```{r categories-example, eval = FALSE}
# Use country groups
data <- md_data(
  ISO = c("BRICS", "G7"),
  formula = "GDP_PC_C",
...
- Academic databases (Jordà-Schularick-Taylor, KOF, etc.)
...
- **macrodata**: Data loading and processing (this package)
